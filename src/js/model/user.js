...
export default class User {
  ...
  async createPatient(userData) {
    ...
    await firebase
      .database()
      .ref("patients")
      .push({ userId: newUserId, package_name: "none" });
  ...
  async getUsers(query) {
    ...
    if (query) {
      users = await firebase
        ...
        .equalTo(query)
        .once("value")
        .then((snapshot) => firebaseLooper(snapshot));
    } else {
      users = await firebase
        .database()
        .ref("users")
  ...
  async getDoctors() {
    ...
    const doctors = await firebase
      .database()
      .ref("doctors")
      .once("value")
      .then((snapshot) => firebaseLooper(snapshot));

    const usersDetails = await firebase
      .database()
      .ref("users")
      .orderByChild("role")
      .equalTo("doctor")
  ...
  async updatePackage(patientId, packageId = 1) {
    ...
    try {
      await firebase
        ...
        .update({ package_name, number_of_visits, expiry });
